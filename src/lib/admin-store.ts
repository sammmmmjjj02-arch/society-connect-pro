...
const initialPayments: AdminPayment[] = [
  ...
  },
  {
    ...
    status: "Rejected",
    rejectionReason:
      "Amount mismatch — ₹1,500 received against a due of ₹2,500.",
...
const initialComplaints: AdminComplaint[] = [
  ...
  },
  {
    ...
    category: "Plumbing",
    description:
      "Seepage on the bedroom ceiling from the flat above during heavy rain.",
    date: "30 Jul 2026",
    status: "Open",
  },
  {
    ...
    category: "Security",
    description:
      "Intercom handset does not connect to the main gate since Monday.",
    date: "27 Jul 2026",
    status: "Open",
  },
  {
  ...
  },
  {
    ...
    description:
      "Wet waste from C wing was not collected for two consecutive days.",
    date: "19 Jul 2026",
    status: "Resolved",
  },
  {
    ...
    category: "Electrical",
    description:
      "11th floor corridor light has been off for four days.",
    date: "11 Jul 2026",
    status: "Resolved",
  },
];
...
export const activity = [
  {
    id: "AA-6",
    text: "Payment PAY-3049 approved for Flat A-303 (June & July 2026)",
    time: "03 Aug 2026, 11:20 AM",
  },
  {
    id: "AA-5",
    text: "Notice “Annual General Meeting — 30 August 2026” published",
    time: "12 Aug 2026, 09:05 AM",
  },
  {
    id: "AA-4",
    text: "Payment PAY-3051 submitted by Flat A-1102",
    time: "12 Aug 2026, 08:12 AM",
  },
  {
    id: "AA-3",
    text: "Complaint AC-1042 moved to In Progress",
    time: "05 Aug 2026, 04:40 PM",
  },
  {
    id: "AA-2",
    text: "Payment PAY-3048 rejected for Flat C-802 (amount mismatch)",
    time: "23 Jul 2026, 10:15 AM",
  },
  {
    id: "AA-1",
    text: "Vehicle MH 12 XY 7788 registered for Flat B-206",
    time: "18 Jul 2026, 06:30 PM",
  },
];
...
export function addResident(input: {
  ...
  const newResident: AdminResident = {
    id: `R-${String(residentSeq++).padStart(2, "0")}`,
    name: input.name.trim(),
    flat: input.flat.trim().toUpperCase(),
    building: input.building.trim().toUpperCase(),
...
export function updateResident(
  ...
  const updated = residents.find(
    (residentRecord) => residentRecord.id === id,
  );

  if (!updated) return;

  updated.name = input.name.trim();
  updated.flat = input.flat.trim().toUpperCase();
  updated.building = input.building.trim().toUpperCase();
  updated.phone = input.phone.trim();
  updated.email = input.email.trim();
  updated.memberSince = input.memberSince.trim();
...
export function restoreResident(id: string) {
  const target = residents.find(
    (residentRecord) => residentRecord.id === id,
  );
...
export function approvePayment(payment: AdminPayment) {
  ...
  state = {
    ...
    ledger: state.ledger.map((row) =>
      row.flat === payment.flat &&
      payment.months.includes(row.month)
        ? { ...row, status: "paid" }
        : row,
    ),
  };
...
export function setAdminComplaintStatus(
  id: string,
...
export function residentPayments(
  ...
  return records
    ...
    .map((m) => ({
      ...
      screenshotName:
        m.screenshotName ?? "payment-screenshot.jpg",
      maintenanceId: m.id,
    }))
    .reverse();
}
