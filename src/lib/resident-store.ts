...
const initialMaintenance: MaintenanceRecord[] = [
  ...
  },
  {
    ...
    rejectionReason:
      "Transaction number did not match the society bank statement. Please re-submit with the correct UTR.",
  },
  {
    id: "2026-04",
    month: "April 2026",
...
const initialComplaints: Complaint[] = [
  {
    id: "C-1042",
    title: "Lift making noise in B wing",
    category: "Lift",
    description:
      "The B wing lift makes a loud grinding noise between the 5th and 7th floor.",
    date: "02 Aug 2026",
    status: "In Progress",
  },
  {
    ...
    category: "Plumbing",
    description:
      "Continuous water leakage near parking slot B-22 since last week.",
    date: "24 Jul 2026",
    status: "Open",
  },
  {
    ...
    category: "Electrical",
    description:
      "7th floor corridor tube light has not been working for four days.",
    date: "11 Jul 2026",
    status: "Resolved",
  },
];
...
export const notices = [
  ...
  },
  {
    ...
    important: true,
    summary:
      "Members with pending dues are requested to clear them at the earliest, oldest month first.",
    body:
      "Members are requested to clear all pending maintenance dues at the earliest. Payments must be made in order, starting from the oldest pending month.\n\nAfter submitting a payment, please allow up to 48 hours for the committee to verify the transaction. A receipt will be available once the payment is approved.",
  },
  {
    id: "N-18",
    ...
    important: false,
    summary:
      "Visitor vehicles must be parked only in the marked visitor bays near the main gate.",
    body:
      "To ease congestion in the compound, visitor vehicles must be parked only in the marked visitor bays near the main gate. Residents are requested to inform their guests in advance and share the vehicle number with the watchman at entry.",
  },
];
...
export const vehicles: Vehicle[] = [
  ...
  {
    number: "MH 12 XY 7788",
    owner: "Anil Kulkarni",
    flat: "B-206",
    phone: "+91 98191 55307",
    type: "Bike",
    model: "TVS Apache",
    parking: "B-11",
  },
...
export function addVehicle(input: {
  ...
  const exists = state.vehicles.some(
    (vehicle) =>
      vehicle.number.replace(/[\s-]/g, "").toUpperCase() ===
...
export function updateVehicle(
  ...
  const newNumber = input.number.trim().toUpperCase();

  state = {
    ...state,
    vehicles: state.vehicles.map((vehicle) =>
      vehicle.number === oldNumber
        ? {
            ...vehicle,
            number: newNumber,
            type: input.type,
...
export function removeVehicle(number: string) {
  state = {
    ...state,
...
export function addComplaint(input: {
  ...
  state = {
    ...
    complaints: [
      {
        id: `C-${1043 + state.complaints.length}`,
        ...input,
        date: today,
        status: "Open",
      },
...
export function approveResidentMaintenance(
  id: string,
  receiptNo: string,
) {
...
export function rejectResidentMaintenance(
  id: string,
  reason: string,
) {
  state = {
